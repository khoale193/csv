...
func (p *Paginator) PageNums() int {
	...
	pageNums := math.Ceil(float64(p.nums) / float64(p.numsPerPage))
	if p.MaxPages > 0 {
...
func (p *Paginator) Page() int {
	if p.Request.Header.Get("X-Paging-Page") == "" {
		return -1 // Cancellation pagination by returning -1
	}
	if p.page != 0 {
		return p.page
	}
	p.page, _ = strconv.Atoi(p.Request.Header.Get("X-Paging-Page"))
...
// PerPageNums return number of item per page
func (p *Paginator) PerPageNums() int {
	if p.Request.Header.Get("X-Paging-Page") == "" {
		return -1 // Cancellation pagination by returning -1
	}
	if p.Request.Header.Get("X-Paging-Size") != "" {
		p.numsPerPage, _ = strconv.Atoi(p.Request.Header.Get("X-Paging-Size"))
	}
	return p.numsPerPage
}
